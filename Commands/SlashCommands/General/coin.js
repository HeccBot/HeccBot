...
export const SlashCommand = {
    ...
    async executeCommand(interaction, api, interactionUser, usedCommandName) {
        ...
        if ( randomCoinFlip === 1 ) { await api.interactions.reply(interaction.id, interaction.token, { flags: MessageFlags.Ephemeral, content: localize(interaction.locale, 'COIN_COMMAND_HEADS') }); }
        else { await api.interactions.reply(interaction.id, interaction.token, { flags: MessageFlags.Ephemeral, content: localize(interaction.locale, 'COIN_COMMAND_TAILS') }); }
