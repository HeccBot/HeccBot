module.exports = {
    ...
    ERROR_INVALID_COLOR_HEX: `That wasn't a valid hex color code! Please try again, using a valid hex color code, including the \`#\` (hash) at the start.`,
...
HeccBot offers a customizable Role Menu module, so you can allow your Server Members to self-assign (or self-revoke) Roles for themselves!
...
- **Swappable:** Members can only grant themselves one Role from a Swappable Menu. If they want another Role on the same Menu, it will be swapped with the previous Role they had.
  - *Useful for Color Roles, for example.*
...
This means that Members would need to have one of the set Required Role(s) before they can use the Role Menu in question. An example use would be locking Color Role Menus to your Twitch Subscribers or Server Boosters.
